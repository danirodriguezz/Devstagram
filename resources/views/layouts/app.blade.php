...
<body class="bg-gray-100">
    <header class="p-5 boder-b bg-white shadow">
        <div class="container mx-auto flex justify-between items-center">
            <a href="{{ route('home.index') }}" class="text-2xl sm:text-3xl font-black ">
                Devstagram
            </a>
            @auth
                <nav class="flex flex-row gap-2 items-center">
                    <a class="text-sm font-bold text-center text-gray-600" href="{{ route('post.index', auth()->user()->username) }}">
                        Hola:
                    ...
                    <div class="flex flex-col sm:flex-row gap-1 items-center">
                        ...
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="text-sm font-bold md:uppercase text-gray-600">Cerrar Sesión</button>
                        </form>
                    </div>
                </nav>     
            @endauth
